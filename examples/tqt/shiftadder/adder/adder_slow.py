    ...
    class cdist(torch.autograd.Function):
        ...
        def backward(ctx, grad_output):
            ...
            if ctx.needs_input_grad[0]:
                X_unsqueeze = torch.unsqueeze(X, 0).expand(
                    W.shape[0], X.shape[0], X.shape[1])
                W_unsqueeze = torch.unsqueeze(W, 1).expand(
                    W.shape[0], X.shape[0], W.shape[1])
                ...
                grad_W = eta * np.sqrt(
                    grad_W.numel()) / torch.norm(grad_W) * grad_W
            if ctx.needs_input_grad[1]:
                grad_X = (torch.nn.functional.hardtanh(
                    (W_unsqueeze - X_unsqueeze), min_val=-1., max_val=1.) *
                          grad_unsqueeze).sum(0)
