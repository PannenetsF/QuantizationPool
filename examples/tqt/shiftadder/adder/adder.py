...
adder_cuda = load(
    ...
    'adder_cuda',
    ['adder/adder_cuda.cpp', 'adder/adder_cuda_kernel.cu'],
    verbose=True)
...
def qg(x, bitsG):
    ...
    if bits >= 32:
        ...
        norm = norm_sign * (norm_int + 0.5 *
                            (torch.sign(norm_float - rand_float) + 1))
        norm = torch.clamp(norm, -factor + 1, factor - 1)
        result = norm / 128
...
class Adder2DFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, weight, kernel_size, stride, padding, eta,
                quantize, weight_bits, quantize_v):
        ...
        adder_cuda.forward(input, weight, output, kernel_size, kernel_size,
    ...
    def backward(ctx, grad_output):
        ...
        eta, kernel_size, stride, padding = (ctx.eta, ctx.kernel_size,
        ...
        if ctx.needs_input_grad[0]:
            ...
            adder_cuda.backward_input(grad_output, input, weight, grad_input,
        ...
        if ctx.needs_input_grad[1]:
            ...
            adder_cuda.backward_weight(grad_output, input, weight, grad_weight,
                                       kernel_size, kernel_size, stride,
                                       stride, padding, padding)
            grad_weight = eta * np.sqrt(
                grad_weight.numel()) / torch.norm(grad_weight) * grad_weight
...
class Adder2D(nn.Module):
    def __init__(self,
        ...
        if self.quantize:
            self.quantize_input_fw = QuantMeasure(shape_measure=(1, 1, 1, 1),
                                                  flatten_dims=(1, -1),
                                                  momentum=momentum)
        ...
        if self.sparsity != 0:
            ...
            self.register_buffer('adder_mask',
                                 torch.Tensor(*self.adder.size()).float())
            self.set_mask()
    ...
    def forward(self, input):
        ...
        if self.quantize is True:
            ...
            if self.quantize_v == 'wageubn':
                self.qadder = round_weight_fixed_point(self.adder,
                                                       self.weight_bits)
            ...
            if self.quantize_v == 'sbm':
                ...
                weight_qparams = calculate_qparams(self.adder,
                                                   num_bits=self.weight_bits,
                                                   flatten_dims=(1, -1),
                                                   reduce_dim=None)
                self.qadder = quantize(self.adder, qparams=weight_qparams)
            bias_fixed_point = None
            output = Adder2DFunction.apply(input_q, self.qadder,
                                           self.kernel_size, self.stride,
                                           self.padding, self.eta,
                                           self.quantize, self.weight_bits,
                                           self.quantize_v)
            if self.quantize_v == 'sbm':
            ...
            output = Adder2DFunction.apply(input, self.adder, self.kernel_size,
                                           self.stride, self.padding, self.eta,
                                           self.quantize, self.weight_bits,
                                           self.quantize_v)
        if self.bias:
    ...
    def round_weight_each_step(self, weight, bits=16):
        ...
        # quantization v2
        weight_qparams = calculate_qparams(weight,
                                           num_bits=bits,
                                           flatten_dims=(1, -1),
                                           reduce_dim=None)
